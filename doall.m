...
R = datasample(xyzMerged,30000);
figure(1)
clf

scatter3(R(:,1),R(:,2),R(:,3),'k.','SizeData',1)
hold on
...
%   plot3(plane(:,1),plane(:,2),plane(:,3),'k.')
  ...
  while stillgrowing
    ...
    hold on
if i == 1
 scatter3(newlist(:,1),newlist(:,2),newlist(:,3),'r.','SizeData',1)
 set(gca,'zdir','reverse');
 save1=newlist;
elseif i==2 
 scatter3(newlist(:,1),newlist(:,2),newlist(:,3),'b.','SizeData',1)
 set(gca,'zdir','reverse');
 save2=newlist;
elseif i == 3
 scatter3(newlist(:,1),newlist(:,2),newlist(:,3),'g.','SizeData',1)
 set(gca,'zdir','reverse');
 save3=newlist;
elseif i == 4
 scatter3(newlist(:,1),newlist(:,2),newlist(:,3),'c.','SizeData',1)
 set(gca,'zdir','reverse');
 save4=newlist;
else
 scatter3(newlist(:,1),newlist(:,2),newlist(:,3),'m.','SizeData',1)
 set(gca,'zdir','reverse');
 save5=newlist;
end
pause(1)
